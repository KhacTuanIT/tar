...
const { Wallets, Wallet, Gateway, User, X509WalletMixin } = require('fabric-network');
...
utils.connectGatewayFromConfig = async () => {
    ...
    try {
        ...
        const idExists = await wallet.get(userid);
        if (!idExists) {
...
utils.events = async () => {
    ...
    var channel = await client.getNetwork(configdata["channel_name"]);
    var peers = channel.getChannel();
    if (peers.length == 0) {
