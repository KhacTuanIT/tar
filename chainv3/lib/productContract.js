...
class ProductContract extends Contract {
    ...
    async createProductLedger(ctx, args) {
        ...
        await ctx.stub.putState(product.code, toBuffer(product));
        ...
        try {
            await ctx.stub.setEvent(EVENT_TYPE, toBuffer(event_obj));
        }
        catch (error) {
    ...
    async testCreateProductLedger(ctx, args) {
        ...
        const res = await ctx.stub.putState(product.code, Buffer.from(JSON.stringify(product)));
        return res.then(r => r.toString());
    }
